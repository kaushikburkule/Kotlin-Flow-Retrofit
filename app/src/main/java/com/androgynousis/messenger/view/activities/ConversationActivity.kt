...
class ConversationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mAdapter = ConversationAdapter(this, setData() as MutableList<ChatDataModel>)
        ...
        send!!.setOnClickListener {
            if (text!!.text.toString() != "") {
                val data: MutableList<ChatDataModel?> = ArrayList()
                val item = ChatDataModel()
                item.time = "6:00pm"
                item.type = "2"
    ...
    private fun setData(): List<ChatDataModel> {
        val data: MutableList<ChatDataModel> = ArrayList()
